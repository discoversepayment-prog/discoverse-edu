...
import { Crown, Zap, Upload, Check, Loader2 } from "lucide-react";
...
export function UpgradeDialog({ open, onOpenChange }: UpgradeDialogProps) {
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      ...
      console.log("QR data from DB:", qr?.value);
      if (qr?.value) {
            ...
            <div className="bg-secondary/50 rounded-xl p-4 flex flex-col items-center gap-3">
              {currentQrUrl ? (
                ...
                <div className="w-48 h-48 rounded-lg bg-card border border-border flex items-center justify-center">
                  <p className="text-[11px] text-tertiary-custom text-center px-4">QR code not available. Contact admin.</p>
                </div>
              )}
